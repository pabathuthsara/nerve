...
**Widened 18 September — the ad platform is now instrumented too, on the same
contract.** `components/meta-pixel.tsx` is the only file that knows the ad
platform exists: keyed or silent, dynamic, `afterInteractive`, every call into
`fbq` wrapped. It fires `CompleteRegistration` on the account submit and
`Lead` on the build screen, and it is deliberately **not** routed through
`capture()` — that catalogue is the product's funnel, read by us; this is an
optimiser's training signal, read by Meta, and the day the pixel is switched
off the funnel has to keep counting.

**Two things there are worth more than the code.** A pixel without a campaign
objective change buys retargeting and nothing else — the optimiser only
optimises for what it is asked for, and the account has been buying taps. And
at $340 of US clicks (~126 at $2.69) a `CompleteRegistration` campaign sees
about four conversions against the ~50 a week it needs to leave the learning
phase, so the campaign should optimise for `Lead` (~15% of clicks, one screen
from the account) until volume exists. `SIGNUP-FIXES-2026-09-18.md` §1.2 and
§5.3.

**Still owed — and all three are keys, not code:** the PostHog project and key,
`NEXT_PUBLIC_META_PIXEL_ID`, the Sentry project and DSN, `withSentryConfig` for
source maps (needs an auth token), and confirmation that the first events
actually land. **Set them and redeploy** — rule 15's second half: a variable
added after a build starts is not in that build. The pipeline half noted below
is unaffected.
...
| D23 | Text mode was the same dating characters typed at, with a meter that could not read what was said (§01, §14) | **Texting is its own section with its own roster, and `/text/` is retired.** Shipped 14 Sep 2026; `docs/TEXTING-PLAN.md` §19 | **Decided and shipped.** The old text mode ran the same model and the same 10 September persona contracts as the voice arm and still read as a character from two weeks earlier, because four things the voice arm does were never wired to it: a meter that reads meaning (it was `start + turns × gain`, so charm and abuse raised warmth identically), reciprocity (`his` was never passed), the two ceilings (`capToBudget` never ran), and rationed standing orders (`includeStanding` defaults true, so every permission she owned arrived before every reply). A fifth was a live bug in both arms: `compileInstructions` re-rolled her mood on **every message**, so she was a different woman every reply — and because the mood block sits near the top of the prompt it also halved the prompt-cache hit. The roster is separate because of the **dials**, not the fiction: every dating trajectory was tuned against ~15 turns in three minutes and a thread runs twenty to forty, so reuse means retuning Tier 0 persona files for a texting reason. **Free narrowed from unlimited to one conversation a day**, spent deliberately in the only window where a takeaway is free — no paying customers, no volume — on the same reasoning as D19's price cut; the cost case does not exist ($0.40/month per free account, measured), the conversion case is the whole case. Plan names, prices and `repsPerDay` are untouched and now pinned. Two things are owed: `isDismissal`'s "get lost in the noise" false positive is **still live on the dating arm**, where it ends a paid rep, and the public pages still describe a product with no texting section |
| D24 | D21's `/start` is one run: ask the track, then the dating focus question, the dating mechanism screen and “what should **she** call you?” — and D1's fix ends the signed-in run at `/interview`, which for a new account is `SetupPrompt` | **The funnel forks at the track answer.** Two step lists on each of the two runs, and the interview one ends **in the free screener brief** rather than on a dashboard. Shipped 16 September 2026 | **Decided 16 September, on a walk-through by the product owner: “I click *for interview practice* and it still takes me on the same onboarding as the dating.”** It did. Every screen worked, and three of the five after the track question were written for the other product — the focus step even apologised for itself in its own sub-heading (*“Your account has both tracks. This one is only about the dating side…”*), which is the tell that a question should not have been asked. **The count is the argument.** Cold account to spoken interview was eleven screens, six of them about dating: `/start`×8 → mic → ready → `/interview` (`SetupPrompt`) → role → CV → questions → interviewer → run setup → brief. It is now eight then four: the role is question two, the CV is question three, and the last step is the interviewer, which starts the round. **What made that possible is one field.** `interview_setups.complete` is *a role title and nothing else* — the CV is optional by design (§C4) — so asking for the role before the account is the whole difference between landing on a free screener and landing on a wizard. `startInterviewSetup` carries it and `seedInterviewSetup` writes it on the service role beside `stampNewAccount`, as an **INSERT** rather than an upsert, so a row that somehow exists is never overwritten. **Three things are load-bearing and each was nearly wrong.** *The two step lists are the same length and differ at exactly one index*, because the run holds the step as an INDEX and somebody who goes back and changes their track answer must stay on the screen they are on — asserted, not commented. *`onboardingResumePath` may read `active_track` only BELOW the track flag*, which is the same trap the function has always existed to avoid: the column carries a database default, so above the flag it is set for everybody who has chosen nothing. *`app/page.tsx` did not select `active_track` at all*, so the one entry point every auth action lands on would have forked to the dating question for every interview account — the E2 shape exactly, a shared function reading a default because its caller did not pass the column that says which product this is; it now also sends a finished interview account to `/interview` rather than to the dating home. **And the round is written on the way out.** `setupFromRow` never answers a null round — it clamps to `DEFAULT_ROUND`, the ten-minute recruiter screen, which costs a credit a new account does not have — so the last step saves `openingRound(hasScreener)` before it navigates. That is B1 caught one screen earlier than B1 found it. **What was refused:** no CV requirement (§C4 degrades to the field, the role and the job description, and a document upload between somebody and the thing they came to practise is the wrong trade at the wrong moment), and no round or difficulty picker in the run (the screener is the only thing the free credit buys, so a picker would be a control with one option). `npm run db:funnel`, `lib/data/guards.test.ts` |
| D25 | §16.4 requires the age gate to run **before the account exists**, and it does: `checkAge` runs on the server ahead of `auth.signUp`, so nothing is created until the date passes | **Google sign-in creates the account first.** §16.4 becomes *gate before the product* on that arm: the account exists, has no date, and `/onboarding/age` is the only screen that will draw until it is given. Shipped 18 September 2026 | **Decided 18 September, and it is the price of the door rather than a defect in it.** There is no way to close it: OAuth returns no date of birth, and the only scope that would ask for one is sensitive — which buys a Google verification review, a security assessment and weeks of waiting, for a login button. So the gate moved rather than went. **What makes it acceptable is what sits between the two moments.** `app/page.tsx` checks `age_confirmed_at` before it checks anything else, and `enforceFrontendGuard` returns early on `/onboarding/age` so no other route renders — a Google account's *first* screen is the gate, and nothing reachable before it spends money, opens a session or meets a character. The guard already had this shape: it was written for accounts created before the gate shipped, and its own comment anticipated the day Google came back. **Two things are load-bearing and neither is the gate.** *The answers cross in a cookie, `sameSite: 'lax'`* — `'strict'` is not sent on the return leg from Google, so the funnel's answers would be dropped on every single sign-up while every test that never left the origin still passed. *The callback redirects to `new URL(next, request.url)` and NOT to `SITE_ORIGIN`* — which reads like a rule 15 violation and is the opposite of one: the session cookies were just written for the host that answered this request, so sending the browser anywhere else lands it signed out. The apex 308s to `www` in the edge before the handler runs, so production is canonical anyway, and localhost and previews are only correct this way. **And one real bug was found in the route nobody had ever reached**: `next` was taken from the query and concatenated, so `?next=//evil.com` built a protocol-relative URL to another host — an open redirect on the one route whose job is to be arrived at from elsewhere. `safeNextPath` and `lib/site/next-path.test.ts`. **What was refused:** no age question added to the OAuth flow before the redirect (it would be asked by a page that cannot yet write anywhere, then asked again by the gate), and no four-colour Google mark (Arena has three semantic colours and the white "G" is permitted by Google's own guidelines). `npm run db:funnel` |
| D26 | Nothing in the repo says whether email confirmation should be on. `signUpWithPassword` handles both: with a session it redirects to `/`, without one it returns `ok` and the screen sends them to `/verify-email` | **Confirmation is OFF at the provider, deliberately, and must stay off.** Production has 30 accounts and 30 confirmations, so every sign-up to date has taken the `data.session` branch straight into the product. Written down 18 September 2026 | **Decided 18 September, on SIGNUP-FIXES §2.5, and it is recorded because it is a one-click change in a dashboard nobody would think to check.** Turning it on inserts an inbox between the account and the first rep — one 2026 analysis of 25,000 SaaS sign-ups lost 8,325 users in exactly that gap — and this product's whole argument is that the free rep is the demonstration, so anything between the form and the microphone is spent at the moment of highest intent and lowest earned belief. **The fallback is correct and stays.** `/verify-email`, the `state.ok` branch and `resendConfirmation` are kept in step with `/signup` on purpose: if confirmation is ever turned on — by us, or by the provider changing a default — the run does not break, it just gets longer. That is a fallback, not a plan. **What this does NOT relax:** §16.4 runs before the account either way (`checkAge` ahead of `auth.signUp`), and an unconfirmed address is still an address we can reach, because `lib/email/` sends the pre-charge notice §14 requires. The risk being accepted is a mistyped address that never hears from us again, which costs that person their streak and costs us nothing a confirmation email would have recovered |
| D30 | §4.4b of the signup audit wanted three real scorecards as social proof, and §4.4/§5.6 both forbid fabricated testimonials outright | **Five named quotes from the five people who tested it, directly below the hero.** Shipped 18 September 2026 | **The process is the whole decision, and it is the reason these are quotes rather than copy.** The product owner asked for five reviews from five friends and supplied the raw sentiment as a combination of what several of them had said. Writing five polished lines from that and attributing them to real people is what §5.6 names as an account-closing risk — so the drafts were shown FIRST, taken to each tester, confirmed as what they actually felt, and only then attributed and shipped. **Drafted, confirmed, then published; never published and then justified.** If any of them ever says "I would not put it like that", their wording replaces ours and no discussion is needed. **What the code enforces, because a true quote can still be dangerous:** `reviews.test.ts` refuses clinical vocabulary in anybody's words (rule 12 and terms clause 08 — a reviewer reading "cured my anxiety" on `/` does not care who typed it), refuses companion framing (the exact category Creem declined this account over), refuses press-release superlatives (a quote made of them has the shape of a bought review and discredits the four beside it that are real), and requires a name on every one — an unattributed testimonial is indistinguishable from an invented one. **The count is derived, never written down**: `reviewCountNote` counts the quotes, the heading carries no number, and `MIN_REVIEWS` hides the entire section below three rather than showing a lonely one, because one testimonial under a headline says nobody else would say anything. **No faces and no stars**: `VISUAL-AUDIT.md` §1 forbids photographs of people as product, a row of stock headshots is the most obvious tell of a bought block, and five stars from five friends is a statistic about nothing |
| D27 | The landing page is silent. `VISUAL-AUDIT` §V2 and SIGNUP-FIXES §4.5 both want proof the software runs, and the one asset that did it — the played-back hero rep — was removed on 10 September | **Ten seconds of voice behind a press, in a sheet, over the real 3D avatar.** Shipped 18 September 2026 | **The product owner asked for a flirty persona welcoming visitors — "practice your conversations against me and other personas", made alluring — and that specific shape was declined and replaced.** The reasoning is `PAYMENTS-APPROVAL.md` §3 verbatim: *a human opens the site during onboarding*, Creem already declined this account once on the AI-companion/dating prohibition (final, no appeal, now disclosable on every future application), and Whop has **no pre-approval gate** — no rejection at the door and no assurance either, so a suspension arrives later with live subscriptions and a balance. A female character personally inviting a visitor to talk to her, on the page a reviewer opens, is rule 9's companion framing exactly. **What shipped instead is a HOUSE voice**, an ElevenLabs voice deliberately not on the roster, because rule 10 says a persona's words must be captured and never authored — so authored words may only be spoken by something that is not a character. The line sells with recognition and stakes (it borrows the page's own headline and lands the scoring law) rather than intimacy. **Three things were found building it.** The orb rendered **crimson**: `visualFor` hashes an unknown name onto a roster row and "Nerve" landed on Nadia's, and Red is semantic in Arena, so the sheet opened looking like an error. The fix is an additive `visual` override on `FluidPersona` plus `lib/site/house-visual.ts` — a new file beside `lib/personas/visual.ts` rather than a tenth row in it, because that table's fallback is `keys[seed % keys.length]` and an extra entry re-rolls the hue of every unknown name in the product (rule 19's shape exactly). `lib/site/house-visual.test.ts` runs the roster's own bounds over it. And **a live product-wide desktop bug** — see D28. **Moved into the hero itself on 18 September**, on the product owner's correction: it was a button that opened a sheet, and what was wanted was the avatar ON the page — filling the right-hand column that §4.5 records as empty at every scroll position, and sitting between the CTA and the rule block on a phone. One DOM order and two layouts (`.hero` is three children now, and the voice spans both rows of column two at 1024px), because a second copy for the other breakpoint would be a second WebGL renderer. **Three rules it is built to:** nothing is ever drawn over the orb — the controls sit beneath it, since an overlaid play triangle hides the middle of the thing it advertises; it **plays once and then offers itself again**, because ten seconds on repeat is a page that will not stop talking to somebody who has moved on to reading; and it starts silent, because browsers refuse audible autoplay and a page that talks at somebody unprompted is worse than one that does not. **And the orb is the page's own grey, which took a measurement to get right.** It shipped cool — a slate blue at 215° — and read as a foreign object; every neutral in Arena is a warm yellow-green in the **77–90°** band (Ground, Surface and Line are all 90°, Ink-3 is 85.7°, Volt is 75.1°), so a blue-grey was 130° from everything else on the screen and could not look like it was made of the same material as the page. It is now Line's hue at Line's chroma. That band is the one `visual.test.ts` keeps every CHARACTER out of, and the reason this may sit in it is the second half of that rule rather than an exception to it: at 0.16 chroma against Volt's 0.83 it cannot read as an accent at any size, which is what the band is protecting. `house-visual.test.ts` asserts the rule rather than the band. The phone orb is capped at 220px rather than 300 for a measured reason: at 390×844 it starts at y=582 and anything larger pushed the PLAY CONTROL past the fold — an orb somebody can see but not start is worse than a smaller one they can. §4.2's CTA position is unchanged at ~395px. **What is still owed:** the captured Cass line that was to follow the intro, which needs a `gpt-realtime` run |
| D28 | Every `Sheet` and `Modal` is centred on desktop by `.sheet-layer { align-items: center }` | **They were all rendering ~97% of their own height too low — mostly below the fold.** Fixed 18 September 2026 | **Found by measuring, not by looking**, while checking whether the D27 sheet's CTA cleared the fold: the layer was correct (top 0, height 900, centred) and the sheet reported `top: 653` with `transform: translateY(97.155%)` stuck on it. **The cause is a hook that cannot know the viewport during the first render.** `useBreakpoint` starts `false` and corrects in an effect — it has to, because reading `matchMedia` while rendering would hydrate differently from the server — so `initial` was ALWAYS captured as the phone's `{ y: '100%' }`. A frame later `isDesktop` flipped and `animate` became `{ opacity: 1, scale: 1 }`, which named no `y`; Framer stopped driving that value and **abandoned it wherever the interrupted entrance had reached**. A motion value nothing animates keeps its last one. The fix is that every state now spans the same keys, so the correction animates instead of stranding. **The phone path is unchanged** — `opacity` and `scale` are already 1 there, so naming them states what was already true, and the measured `sheetTop` is identical before and after (404 at 390×844). Desktop now centres at exactly `(900−430)/2 = 235`. This affected `EndRepModal`, `PaywallSheet`, `HowItWorksSheet`, the distress sheet and the persona detail sheet — every modal in the product, on every desktop browser |
| D29 | `/admin` answers "did anybody come, and did they sign up" from first-party data it owns | **It could not answer where inside `/start` they stopped**, which is the one number D21 exists for. Fixed 18 September 2026 | **`page_views.path` is a route SHAPE and `/start` is one route with eight screens behind it** — the run holds its position in React state and never touches the URL — so "Where they landed" reported `/start` as a single row and the per-step drop was invisible to everything except PostHog's `start_step_viewed`, which has never been keyed. A nullable `step` column, an `admin_start_funnel` RPC and a `FunnelTable` close it. Three things there are deliberate. **The step is validated against an authored list, not sanitised** — it arrives from a browser that can post anything, and a free-text column reachable from the network is a log of whatever somebody sends; a forged value is dropped and the view is still counted. **The list is repeated rather than imported**, because this module is the boundary a browser posts into and must keep meaning the same thing when a screen is renamed — `pageview.test.ts` asserts the two agree, so the drift is caught at build time instead of being possible. **And the ORDER comes from `startSteps`, never from SQL**, so a reorder of the run cannot leave a stale sequence in a migration; the rows are drawn in run order rather than sorted by size, because sorting by volume is right for paths and destroys the only thing a funnel table is for. Percentages are of the FIRST screen throughout — step-to-step figures read more dramatically and answer the wrong question. **It does not replace PostHog** and is not meant to: `capture` and the beacon both fire, because the whole reason D21 could not be argued about is that the only instrument for it was the one nobody had turned on |
...
| D13 | "No coaching during the rep. Nothing on screen but a timer, a live waveform and the mission" (§05); "not a reply generator — we never write your messages for you… our entire differentiation is the opposite promise" (§01) | **Tess only** carries an on-screen script during her rep: an aim, and for five of the six scored dimensions an example line to say | **Decided 6 September in favour of the build, bounded to one character.** Tess is rung 1 and is who a new account meets on the one free rep, and first-session drop-off is where products in this category die — a user who freezes there never finds out that rung 2 is better. `site-audit-openai.md` reached the same conclusion from outside ("a performance dashboard without a coach") and put "a guided first win" on its borrow list. **What keeps it bounded is code, not this note:** `lib/data/guided.ts` holds one script for one slug; `assertGuidedStep` is stricter than anything else in the build and refuses appearance, pickup, contact-detail and ask-her-out vocabulary outright; `assertNoScript` and every mission on the other nine surfaces are untouched; and `guided.test.ts` walks the real roster to assert that exactly one persona carries the flag. The live rail is a rail and never a pop-up — text changing in place, `aria-hidden`, no animation — because §05's objection is to *interruption*, and the whole script is read on the brief first, where coaching has always been allowed. **The public claim moved with it**: `components/site/landing.tsx` now says "Your first character walks you through it. After that the words are yours", because §14 has a merchant-of-record reviewer reading that page and a promise the build no longer keeps is the kind of thing that gets checked. **Two defects found on the first real guided rep, 6 September, both fixed the same day.** The rail advanced on his turn count alone, so when her first two replies never reached him (B11) step two told him to *"follow one answer twice"* against a silence containing no answer — he read the line out, and that utterance is what aborted the reply she was still synthesising. It advances on completed **exchanges** now (`min(userTurns, agentTurns)`), so it cannot point at an answer that has not arrived, and a failing pipeline holds the script still instead of marching somebody through a monologue. Separately the close sat at six user turns, which the pipeline arm reaches in about a minute: the rail told him to leave at 0:63 of a 3:00 rep, he read the line, and the one free rep a new account gets ended at 74 seconds in a phone number. **The close is now reachable only through `wrapping`** — the format already owns that moment at `WRAP_UP_MS`, and telling him to leave before she has been told to wind down is the product arguing with itself, the same argument `dueSceneBeat` settles with `LAST_BEAT_FRACTION`. **Three more defects, found 11 September by the product owner watching the feature rather than the tests, all fixed the same day — and two of them were not cosmetic.** *(i) The rail ran out in the first minute.* `STEP_AT` was `[0, 1, 2, 4, 7]`, authored against "the twelve-to-fourteen exchanges a three-minute rep contains"; the arm that ships completes an exchange about every ten seconds — the same measurement that produced the 0:63 close above — so four of the five steps were spent inside the first seventy seconds and `composure`, the only step with no line, then held the rail for the remaining eighty. A rail that stops changing halfway through does not read as a last lesson, it reads as a broken feature, which is how it was reported. It is `[0, 1, 3, 5, 9]` now, roughly a step every twenty-five seconds, with `composure` landing in the last third — the only part of a rep where a silence is a live risk rather than a hypothetical. The gate is still **exchanges** and never the clock; the numbers are just no longer bunched at the start. *(ii) One authored line coached a dead end.* `signalReading` read **"You look like you are in a hurry."** — said to a character who has decided to stay in a gallery until she finds one painting she likes. It is false in her room and false in the one direction that costs: `lib/warmth/reciprocity.ts` prices a dead end above what a good question earns, so the single line in the product most likely to lose a beginner their first rep was one of ours. It is now a read of her *reaction* — it opens instead of closing, and `why` already said the skill works whether you were right. Both are asserted as **properties** in `guided.test.ts` rather than as copies of the table. *(iii) The brief lectured.* Six numbered steps, each with a direction, a line and a sentence of rationale, under a heading reading *"Guided rep · she will walk you through it"*, over a three-sentence footnote — around a hundred and fifty words immediately before the first time anybody in this product opens a microphone, and above a mission note, a rule block and a technique card that were already there. It explained the mechanism instead of naming the thing, and it spent the prompts before they could land. It is a **signifier** now: a badge, the six dimension marks the scorecard will reuse, one sentence, and the line to open with. The rest is behind a closed disclosure, which is not decoration — the live rail is `aria-hidden`, so the brief is the screen-reader path and the whole script had to stay in the DOM. It also stands **instead of** `MissionNote` rather than above it; a brief carrying a mission *and* six prompts handed a first-time user two sets of objectives for the same three minutes, which is the round-6 failure wearing an interface and the one thing the live screen had always refused. **And the rail floats.** It was a centred label in the middle of the stack, indistinguishable from the `MissionLine` it replaces; it is a card now — its own surface, a hairline, a blur, a scrim — lifted out of the flow and anchored to the floor, which is where a teleprompter belongs and keeps the orb clear. §02 survives because **floating is a position and not a movement**: a step change is still a cross-fade in place, six pixels of rise, 280ms, and `prefers-reduced-motion` removes even that. Anchoring it to the floor rather than the stack is also what stops the orb jumping every time a two-line prompt follows a three-line one. Three tiers — the dimension (the scorecard's own word, read from `SUB_SCORE_LABELS` and never re-authored), the aim, the line — and a step with no line promotes its aim into the line's slot instead of leaving the space empty. The bracketed blank is drawn as a blank (`splitSay`) rather than reaching the screen as two literal brackets explained in a footnote on another screen. **Two more from the same watch, and they are the interesting half.** *(iv) One line per step is too few, in two different ways.* Five reachable steps across the fifteen-or-so exchanges a rep contains is a prompt that stands for twenty-five seconds at best — reported as *"staying at one hint for a long time"* — and a single authored sentence per lesson has to be general enough to survive anything she might have said, which makes it general enough to fit nothing in particular. A step now carries a **set** of lines, offered one per exchange spent on it and holding on the last, so the rail changes on most exchanges of a rep rather than four times in the first seventy seconds (asserted as a cadence property, not as a copy of the table). The sets are ordered the way a conversation earns them: `curiosity` opens on the room, because at the first exchange she has answered an opener about a painting and a line asking what something is *like* has nothing to attach to, and only then moves to the canonical follow-up. **`composure` gained a second beat and its first one still has no line** — the header's argument was that if a line is ever wanted there, the thing to change is the LESSON, so the lesson changed: composure is not filling a silence *and then coming back without apologising for it*, and the second half has a sentence ("Let me think about that for a second."). Without it the step that holds the last third of the rep never changed at all. *(v) The ladder is blind, and there is exactly one moment where that is harmful rather than merely imprecise.* **She has asked him a question and the rail is telling him to ask one.** Every other mismatch is a prompt that fits badly; this one talks over her. `ANSWER_HER` is a single authored prompt that outranks the ladder whenever hers is the last turn in the transcript and it asked something — suppressed on the opening, and never over the wind-down, which stays first for the 6 September reason. **It carries no line, and that is the design**: what he should say is the answer to a question we have not read, so it renders as the direction alone, promoted, exactly as `composure`'s first beat does. `herLastTurnAsked` means *she is waiting*, not *she once asked* — reading the last agent turn instead of the last turn would have left the prompt standing for another whole exchange after he answered, which is the defect above wearing the fix's clothes. The rail's position dots keep showing the LADDER while it is up: the ladder has not moved, and the dots answer "where are we in the rep". **This is the only thing in the rail that reads the conversation, it is a boolean and never her text, and nothing downstream may compose a sentence out of it** (rule 10). **And the connecting beat got a screen.** "Connecting · she can't hear you yet" was a 9px label under a fully-drawn avatar — so the first thing a new account sees is her, present and apparently listening, while the transport is still opening and nothing they say reaches anything. She is not drawn until she can hear you now: a full-bleed hold on the same radial ground the 3·2·1 uses, carrying a **skeleton of the orb** (§02's answer to a spinner — the arriving content is a circle, so that is what breathes there) and the sentence with her name in it rather than "she", which was also wrong on the interview arm. `--connecting` joins `--arming` on the same dim, so the count and the wait are one continuous hold and her arrival is a single 620ms fade at the end of whichever finishes last. Because `onGo` fires at the **top** of the count, most connections are open before the last tick and this is never seen. **Owed:** none of this has been heard or seen on a real rep with a microphone, which is what the whole pass was reported off and the only thing that can settle the retimed ladder — the cadence is reasoned off an arm that completes an exchange about every ten seconds, not watched. And whether guided reps should be comparable to unguided ones on the personal-best composite is an open question — Tess is rung 1 and already the easiest, so it introduces no new comparability problem the ladder did not already have |
...
| D21 | Nothing in §11 or §14 describes how a stranger reaches an account; the build had one route — `/signup`, an email field, reached by pressing **Start training free** on `/` | **`/start`**, an eight-screen run that asks the three onboarding questions BEFORE the account and carries the answers into it. Shipped 11 September 2026 | **The evidence, and it is thin on purpose.** Twenty-seven visits arrived from TikTok and fifteen from Meta between 4 and 10 September and **none of the forty-two created an account**. Nothing in the product could say where they left: `lib/analytics/events.ts` began at `brief_viewed`, which is past sign-up, so the entire acquisition half of the funnel was unmeasured — that is now four events (`start_step_viewed`, `start_answered`, `start_account_submitted`, `start_account_failed`) and the catalogue test asserts they sit **before** `brief_viewed` so they cannot be dropped as noise later. Note what 0/42 does and does not prove: a true 3% signup rate produces zero on 42 visits about a quarter of the time, so this is a bet on a diagnosis rather than a measurement of one, and **2 signups after it is not evidence that it worked**. §9 of `MARKETING-PLAN.md` is where the number goes. **What was refused, and why each refusal is load-bearing.** *A rep before the account* — asked for directly: every route that spends money goes through `maySpend`, which is keyed to a user id (rule 11), so an anonymous rep is an uncapped voice spend with no owner; and §16.4 requires a date of birth before anybody speaks to a character, so it is also a dating conversation with an unverified age, which is precisely what `PAYMENTS-APPROVAL.md` is about. *A paywall at the end of the run* — the genre convention (Rizz puts one at screen 20 of 63; Cal AI at the end of 32): ours would sell at the moment of lowest earned belief, when the free rep costs us ~8¢ and is the most convincing thing we own. *A statistic on the reframe screen* — every number about people avoiding conversations is one clause from a prevalence claim, which rule 12 and terms clause 08 both forbid; the screen is copy only. *Serving the funnel from `/` behind a UTM check* — that is cloaking, and it would make the compliance review non-deterministic. **`/` is unchanged and stays the page a merchant-of-record reviewer opens** (B1). What moved is every public CTA — the landing hero and foot, the header, the footer, `/how-it-works`, `/pricing`, `plan-board`, and `/interviews` (which sends `?track=interview`, pre-answering question one). `/signup` keeps exactly one caller, `pack-buy.tsx`, because a purchase names the pack on that screen. **One door to an account, one conversion rate to read.** `npm run db:funnel` |
| D22 | §14 prices voice by the month and the week; nothing describes a period longer than one, and the affiliate commission was **40% global / 50% member**, set in a script and read by no test | **Pro by the year at $149**, seven-day trial, twenty-four interview credits **dripped two a month**. Commission cut to **30% / 30%**. Shipped 13 September 2026 | **Decided 13 September, and the commission is the half that mattered.** The year exists for the affiliate cheque rather than the buyer's discount: 30% of a $19 month is $5.70, dripped monthly against a subscription that realistically lives a quarter, and course affiliates in this niche make $200-400 on one sale — so a cold email offering less than lunch is why the first creators never replied. $149 pays **$44.70 once, on the day it happens**. **The rates were wrong in a way nothing could have reported.** Every rung has to clear its cost of goods at the usage it *permits*, not the usage we expect: `repsPerDay` is a promise already made. At 50% the year sold about $25 underwater and monthly Pro netted pennies; at 40% the year still lost inside its own cap. That subtraction was done by hand twice and **got it wrong the second time**, because reps × $0.08 silently omits the twenty-four interview credits the same plan grants — 35% looked safe and is $3.61 negative. So it is computed now: `lib/billing/economics.ts` holds the rates, the fee model and the ceiling, `economics.test.ts` asserts *no offer loses money at any rate we pay at maximum usage*, and `whop:verify` reads the live percentages back off the product — the one class of number at the provider that can turn a completed sale negative, previously checked by nobody. The year is the binding rung at **32.6%**, which is why the member premium was dropped rather than kept. **The credits are dripped and that is what makes the trial safe.** One payment, one grant is the rule that holds for a week and a month and breaks on a year: granting twenty-four at once lifts `voice_daily_cap_cents` from 300c to **2,460c** for twelve months — §19's shared dial reached through DATA, at twelve times the size that caught free's cap — and a card-backed trial emits a real `payment.succeeded` at **$0**, so the whole year would go to somebody who has paid nothing and can cancel on day six. `creditGrants` states how many grants a period holds, the webhook still writes the first, `/api/cron/interview-credits` writes the rest. `dripsOwed` names **every** slot the period has reached rather than the newest, so a missed run self-heals and the ledger's unique reference refuses the duplicates. **Adding a period found four latent bugs, all of the same shape — a binary where the code had assumed two.** `setup-whop.ts` would have created the plan at the provider titled **"Nerve Pro Monthly"**, a $149-a-year plan wearing the $19 plan's name on the buyer's statement. `notify.ts` passed `plan.price` to the one email sent before a card is charged, so it announced **$19** for a $149 charge, and `trial.ts` called it *"the first month"* — §14's exact failure, committed by the message that exists to prevent it. `interviewsLine` printed **"2 / year"** for a plan granting twenty-four. `billingDaysFor` errs long on purpose and its cost analysis was written against a 30-day worst case; a year made it 365 without touching the file, so an unresolvable grant would have lived twelve times too long. None is about the year: each is a two-valued assumption that was correct until it wasn't. **Shipped to production the same day** as `dpl_GcLYxzVr8NqhtS3uwYsX5SJ2VLk8` off `ae3a1b8`, git-linked and clean: `/pricing` serves three tabs, the cron route answers 401 unauthenticated, and `whop:verify` reads back 0 failed with the classification intact after the product write. `CRON_SECRET` was already set in production and had been since 24 August — it was only blank in `.env.local`, which is what the preflight warning was about. "
"**Two provider facts were then confirmed by hand and both mattered.** *Commission does not recur* — it is paid on the FIRST payment only, which no payload this repo can read exposes, and the generated brief was promising **"30% of every payment, for as long as your referral keeps paying"** to the one document a creator reads before posting. Corrected, and the correction is the better pitch: with nothing trailing, what a referral is worth is decided entirely by which plan the buyer picks — $5.70 on a month against $44.70 on a year, **about 8x for the same referral**. It also means `marginAt` models the worst period rather than a typical one, since period one bears the whole commission and every renewal bears none. *Adaptive pricing stays on* — the audience is US-first, so list price is what most buyers meet, and the $2 charge that first suggested otherwise was a hand-set test price rather than a purchasing-power discount. |
