...
| [`PAYMENTS-WHOP.md`](PAYMENTS-WHOP.md) | **How Creem was replaced with Whop. The code shipped 1 September; the account setup has not.** Chosen 1 September over Gumroad because it is the cheap migration: the same Standard Webhooks signing Creem used, a checkout API that carries our `user_id` through to the membership, a real sandbox, and a server-side cancel that finally makes `TRIAL_NOTE`'s promise true. Manual setup in order, the code file by file, the event mapping, the ten sandbox tests, and the go-live checklist |
...
| The trial — its length, when the card is charged, or how somebody cancels | `TRIAL_DAYS` and `TRIAL_NOTE` in `lib/site/plans.ts`, the `trial_period_days` on both Whop plans, **and** `components/site/legal-pages.tsx` §07 in the same edit. All three are the same promise and the legal page is the one a disputing customer quotes back |
