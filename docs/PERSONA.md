...
| `moods` | **three per character, roster-wide**; adds `# Today, specifically` | The steering line is deterministic in warmth, so a rep inside one band carries one instruction start to finish. A mood changes what she has to talk about and **never** what she gives |
| `room.place` | `sceneId(room)` in prose | The scene id is an audio lookup and the word for where she is standing is not. Every character now has her own authored scene, so the two agree — but `place` stays set wherever the id would not read as English in "a stranger in a ___" ("in a train platform" is not a sentence), and it is what `scorerPlaceFor` anchors the live intimacy scorer to. Before 6 September nine characters shared two scenes and seven of them fell through to somebody else's room (`PERSONA-AUDIT.md` §3.6) |
