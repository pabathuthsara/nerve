...
import { useInterviewers, usePersonas, useScorecard, useSession, useTranscript, useUserState } from '@/lib/data'
import type { Band, JudgementBand, MetricBand, Moment, SessionSummary, TranscriptTurn } from '@/lib/data/types'
import { ARM_THRESHOLD, LEVEL_NAMES } from '@/lib/data/progression'
import { AppShell } from '@/components/app-shell'
import { Avatar, Button, Card, Chip, EmptyState, LockOverlay, ProgressRing, Skeleton, Tabs } from '@/components/ui'
...
function ResultScreen({ session }: { session: SessionSummary }) {
  ...
  const threshold = session.track === 'interview' ? 70 : ARM_THRESHOLD
  const close = threshold - session.finalWarmth
  const context = session.track === 'interview'
    ...
    : close > 30 ? "She wasn't interested from the start. Some aren't." : close >= 10 ? (session.finalWarmth >= 40 ? 'You had her attention and lost it.' : 'She never really opened up.') : 'You were close.'
  ...
  return <main className={`result-page result-page--${session.won ? 'win' : 'loss'}`}><Avatar name={subject?.name ?? session.personaName} src={subject?.portraitUrl} size={80} dimmed={!session.won} /><h1 className={`display-xl${session.won ? ' volt' : ''}`}>{headline}</h1>{session.won ? <><span className="result-time data">{formatDuration(session.durationMs)}</span><Chip tone="band" band={session.finalBand}>{session.track === 'interview' ? interviewBand(session.finalBand) : session.finalBand}</Chip></> : <><div className="result-ring"><ProgressRing value={session.finalWarmth / threshold * 100} size={86} strokeWidth={4} color={bandCss(session.finalBand)} /><span className="data">{session.finalWarmth} / {threshold}</span></div><p>{context}</p></>}<div className="result-actions"><Link className="arena-button arena-button--primary arena-button--lg arena-button--full" href={`/session/${session.id}/scorecard`}>See breakdown</Link><Link className="arena-button arena-button--ghost arena-button--full" href={repHref}><RotateCcw size={17} strokeWidth={1.5} /> Run it back</Link></div><FirstWinSheet open={firstWin} onClose={closeFirstWin} /></main>
}
