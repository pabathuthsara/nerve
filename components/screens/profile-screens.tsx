...
import { Check, ChevronRight, Download, Headphones, LogOut, Mic, Trash2 } from 'lucide-react'
import { useCallback, useEffect, useMemo, useRef, useState } from 'react'
import { useLifetimeStats, useSessionHistory, useUserState } from '@/lib/data'
import type { Plan, SessionSummary } from '@/lib/data/types'
import { signOut } from '@/app/auth/actions'
import { saveAudioPreferences, saveDisplayName, saveTrainingWheels } from '@/app/profile/actions'
...
function ProfileHome() {
  ...
  return <AppShell title="Profile"><div className="profile-head">{userLoading || !user ? <><Skeleton width={64} height={64} style={{ borderRadius: '50%' }} /><div style={{ flex: 1 }}><Skeleton width={170} height={34} /><Skeleton width={210} height={14} style={{ marginTop: 8 }} /></div></> : <><Avatar name={user.displayName} size={64} /><div><h1 className="display-lg">{user.displayName}</h1><span className="muted">{user.email}</span></div><Chip tone="volt">{user.plan}</Chip></>}</div><section><span className="label">Lifetime stats</span>{statsLoading || !stats ? <div className="profile-stats">{Array.from({ length: 6 }, (_, index) => <Skeleton key={index} height={92} />)}</div> : <div className="profile-stats"><Stat label="Total reps" value={stats.totalReps} /><Stat label="Win rate" value={stats.winRate === null ? '—' : `${stats.winRate}%`} /><Stat label="Best time" value={stats.bestTimeMs === null ? '—' : formatTime(stats.bestTimeMs)} /><Stat label="Avg warmth gain" value={stats.averageWarmthGain === null ? '—' : `${stats.averageWarmthGain > 0 ? '+' : ''}${stats.averageWarmthGain}`} /><Stat label="Current streak" value={`${stats.currentStreak} days`} /><Stat label="Longest streak" value={`${stats.longestStreak} days`} /></div>}</section><Card className="profile-chart-card"><div className="card-heading"><div><span className="label">Last 20 sessions</span><h2 className="display-md">Warmth over time</h2></div><div className="chart-legend"><span><i /> Warmth</span><span><i className="cool" /> Score</span></div></div>{sessionLoading ? <Skeleton height={150} /> : <WarmthChart sessions={sessions} />}</Card><nav className="profile-links"><ProfileLink href="/profile/history" label="Session history" /><ProfileLink href="/profile/subscription" label="Subscription" /><ProfileLink href="/profile/settings" label="Settings" /></nav><Button variant="ghost" onClick={() => setSignOutOpen(true)}><LogOut size={18} strokeWidth={1.5} /> Sign out</Button><Sheet open={signOutOpen} onClose={() => setSignOutOpen(false)} title="Sign out?"><div className="sheet-stack"><p>Your saved reps stay exactly where they are.</p><form action={signOut}><Button type="submit" fullWidth>Sign out</Button></form><Button variant="ghost" fullWidth onClick={() => setSignOutOpen(false)}>Stay here</Button></div></Sheet></AppShell>
}
