...
export function RosterScreen() {
  ...
  return <><div className="screen-heading"><span className="label">Progression map</span><h1 className="display-lg">Roster</h1><p>Read the room. Then earn a harder one.</p></div>{!loading && personas.length === 0 ? <EmptyState mark="state-roster" title="The roster is empty" description="New training partners will appear here." /> : <div className="level-list">{loading ? levels.map((level) => <RosterSkeleton key={level.level} />) : levels.map((level) => <LevelSection key={level.level} config={level} personas={personas.filter((persona) => persona.level === level.level)} progress={progress} />)}</div>}</>
}
...
export function PersonaDetailScreen({ personaId }: { personaId: string }) {
  ...
  if (loading) return <><div className="persona-detail"><Skeleton height={560} /></div></>
  if (!persona) return <><EmptyState mark="state-roster" title="Nothing here" description="That person is not on the roster." action={<Link className="arena-button arena-button--primary" href="/roster">Back to roster</Link>} /></>
  ...
  return <><Sheet open onClose={() => router.push('/roster')} title={persona.name}>{content}</Sheet></>
}
