...
export function BaselineScreen() {
  ...
  if (loading) {
    return <><div className="screen-heading compact"><Skeleton width={220} height={34} /></div><Skeleton height={320} /></>
  }

  if (!state) {
    return <><EmptyState mark="state-chart" title="No baseline yet" description="Your first graded rep becomes the measurement everything after it is compared against. Run one." action={<Link className="arena-button arena-button--primary" href="/train">Start a rep</Link>} /></>
  }
