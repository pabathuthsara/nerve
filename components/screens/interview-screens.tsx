...
import { Button, Card, Chip, EmptyState, FileDrop, Input, ProgressBar, Skeleton, Stat, Textarea } from '@/components/ui'
...
export function InterviewerPicker() {
  ...
  return <AppShell title="Interviewers"><div className="screen-heading"><span className="label">Choose the pressure</span><h1 className="display-lg">Your interviewer</h1><p>Style changes the questions, follow-ups, and tolerance for vagueness.</p></div>{!loading && interviewers.length === 0 ? <EmptyState mark="state-roster" title="No interviewers yet" description="The next interviewer is being prepared." /> : <div className="interviewer-grid">{loading ? Array.from({ length: 4 }, (_, index) => <Skeleton key={index} height={300} />) : interviewers.map((interviewer) => <button key={interviewer.id} className={`interviewer-card${interviewer.locked ? ' locked' : ''}${selectedInterviewerId === interviewer.id ? ' selected' : ''}`} aria-pressed={selectedInterviewerId === interviewer.id} disabled={interviewer.locked} onClick={() => { setSelectedInterviewerId(interviewer.id); router.push('/interview') }}><div className="interviewer-portrait"><FluidPersona name={interviewer.name} personaId={interviewer.id} warmth={16} fill />{interviewer.locked ? <span className="label">Level 4</span> : null}</div><div><Chip tone={interviewer.locked ? 'neutral' : 'volt'}>{interviewer.styleLabel}</Chip><h2 className="display-md">{interviewer.name}</h2><span className="label">{interviewer.gender} · level {interviewer.level}</span><p>{interviewer.blurb}</p></div>{!interviewer.locked ? <span className="select-line"><Check size={15} strokeWidth={1.5} /> {selectedInterviewerId === interviewer.id ? 'Selected' : 'Select'}</span> : <span className="select-line">Locked</span>}</button>)}</div>}</AppShell>
}
